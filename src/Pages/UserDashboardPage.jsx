...
const UserDashboardPage = () => {
    ...
    const fetchRequestedMeals = async () => {
      try {
        const res = await axios.get(
          `http://localhost:3000/requests/${userData.email}`
        );
  ...
  const handleCancel = async (mealId) => {
    ...
    try {
      const res = await axios.delete(
        `http://localhost:3000/requests/${mealId}`,
  ...
  useEffect(() => {
    ...
    const fetchReviews = async () => {
      try {
        const res = await axios.get(
          `http://localhost:3000/reviews/user/${userData.email}`
        );
        setReviews(res.data);
      } catch (err) {
        console.error(err);
  ...
  useEffect(() => {
    ...
    const fetchPayments = async () => {
      try {
        const res = await axios.get(`http://localhost:3000/payments/${userData.email}`);
